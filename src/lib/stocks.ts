...
// Last-resort static defaults used only when Yahoo discovery/ranking fails.
const STATIC_FALLBACK_TRENDING_SYMBOLS = ['RELIANCE.NS', 'TCS.NS', 'HDFCBANK.NS', 'INFY.NS', 'ICICIBANK.NS'];
const TRENDING_CANDIDATE_SYMBOLS = [
	'RELIANCE.NS',
	'TCS.NS',
	'HDFCBANK.NS',
	'INFY.NS',
...
export async function getTrendingSymbols(limit = 6) {
	try {
		const searchResults = await Promise.all([
			yahooFinance.search('NSE') as unknown as SearchResult,
			yahooFinance.search('BSE') as unknown as SearchResult,
			yahooFinance.search('India stocks') as unknown as SearchResult,
		]);

		const discoveredSymbols = searchResults
			.flatMap((result) => result.quotes || [])
			.map((quote) => quote.symbol)
			.filter((symbol): symbol is string => Boolean(symbol && (symbol.endsWith('.NS') || symbol.endsWith('.BO'))));
		...
		console.warn('Falling back from Yahoo trending search:', error);
	}

	try {
		const quotes = await Promise.all(
			TRENDING_CANDIDATE_SYMBOLS.map(async (symbol) => await getStockQuote(symbol))
		);

		const rankedSymbols = quotes
			.filter((quote): quote is NonNullable<Awaited<ReturnType<typeof getStockQuote>>> => Boolean(quote?.symbol && quote.price))
			.sort((a, b) => {
		...
		if (rankedSymbols.length > 0) {
			return rankedSymbols.slice(0, limit);
		}
	} catch {
		// Fall through to static defaults
	}

	return STATIC_FALLBACK_TRENDING_SYMBOLS.slice(0, limit);
}
...
export async function getStockNews(symbol: string) {
	try {
		// weird linting issue
		const result = await yahooFinance.search(symbol) as unknown as SearchResult;
...
export async function searchStocks(query: string) {
	try {
		const result = await yahooFinance.search(query) as unknown as SearchResult;

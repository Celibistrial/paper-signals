...
import { ArrowLeft, Tag, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
const SECTOR_STOCKS: Record<string, string[]> = {
  ...
  'Consumer Defensive': ['HINDUNILVR.NS', 'ITC.NS', 'NESTLEIND.NS', 'BRITANNIA.NS', 'TATACONSUM.NS'],
};

export default async function SectorPage({ params }: { params: Promise<{ name: string }> }) {
  const { name: encodedName } = await params;
  const name = decodeURIComponent(encodedName);
  const symbols = SECTOR_STOCKS[name] || ['RELIANCE.NS', 'TCS.NS']; 
